...
# Kvantum Theme Installation and Configuration Script for Fedora
...
print_status "Starting Kvantum theme installation and configuration for Fedora..."
...
KVANTUM_PACKAGES=(
    "kvantum"                        # Kvantum theme engine (Qt5/Qt6)
...
GTK_PACKAGES=(
    "gtk-murrine-engine"             # Murrine GTK theme engine
    ...
    "liberation-fonts"               # Liberation fonts (metric-compatible with Windows fonts)
    "google-noto-sans-fonts"         # Google Noto Sans fonts
    "fontconfig"                     # Font configuration library
    "google-noto-sans-mono-fonts"    # Google Noto Mono fonts
...
for package in "${GTK_PACKAGES[@]}"; do
    if rpm -q "$package" &>/dev/null; then
        ...
        print_status "Installing $package..."
        sudo dnf install -y "$package" || print_warning "Failed to install $package (might not be available in official repos)"
    fi
done
...
ICON_PACKAGES=(
    "elementary-xfce-icon-theme"     # Elementary-style icon theme
...
for package in "${ICON_PACKAGES[@]}"; do
    if rpm -q "$package" &>/dev/null; then
        ...
        print_status "Installing $package..."
        sudo dnf install -y "$package" || print_warning "Failed to install $package (might not be available)"
    fi
done
...
ADDITIONAL_FONTS=(
    "liberation-fonts"               # Liberation fonts (already in main list but checking again)
    "google-droid-sans-fonts"        # Droid fonts
    "gnu-free-fonts-common"          # GNU FreeFont family
)
...
for package in "${ADDITIONAL_FONTS[@]}"; do
    if rpm -q "$package" &>/dev/null; then
        ...
        print_status "Installing $package..."
        sudo dnf install -y "$package" 2>/dev/null || print_warning "Failed to install $package (might not be available)"
    fi
done
...
main() {
    ...
    print_status ""
    print_status "Note: Some packages (arc-theme, numix-icon-theme, elementary-xfce-icon-theme) may"
    print_status "require the RPM Fusion or a third-party COPR repository to be enabled on Fedora."
}
